...
public class AlarmKlaxon extends Service {
    ...
    private Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case KILLER:
                    Log.v("guo", "*********** Alarm killer triggered ***********");
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (alarm == null) {
            Log.v("guo", "AlarmKlaxon failed to parse the alarm from the intent");
            stopSelf();
            return START_NOT_STICKY;
        }
    ...
    private void play(Alarm alarm) {
        ...
        Log.v("guo", "AlarmKlaxon.play() " + alarm.id + " alert " + alarm.alert);
        ...
        if (!alarm.silent) {
            ...
            if (alert == null) {
                ...
                Log.v("guo", "Using default alarm: " + alert.toString());
            }
            mMediaPlayer = new MediaPlayer();
            mMediaPlayer.setOnErrorListener(new OnErrorListener() {
                public boolean onError(MediaPlayer mp, int what, int extra) {
                    Log.v("guo", "Error occurred while playing audio.");
                    mp.stop();
                    mp.release();
                    mMediaPlayer = null;
                    return true;
            ...
            try {
                if (mTelephonyManager.getCallState()
                    ...
                    Log.v("guo", "Using the in-call alarm");
                ...
                Log.v("guo", "Using the fallback ringtone");
                try {
                    mMediaPlayer.reset();
                    setDataSourceFromResource(getResources(), mMediaPlayer,
                            R.raw.fallbackring);
                    ...
                    Log.v("guo", "Failed to play fallback ringtone"+ex2);
                }
            }
        }
        if (alarm.vibrate) {
